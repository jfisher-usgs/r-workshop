...
As John Chambers ([2008](http://www.springer.com/gp/book/9780387759357)) states:
...
There are a standard set of packages included with an installation of R and many more are available through the
...
Use the `install.packages` function to install packages from CRAN-like repositories.
To install the **inlmisc** package from CRAN, use the command

```{r, eval=FALSE}
install.packages("inlmisc")
```

By default `install.packages` also installs packages which **inlmisc** depend on/link to/import,
see the [package description](https://CRAN.R-project.org/package=inlmisc) for its dependencies.
Installing packages from a different CRAN-like repository,
such as the **G**eological survey **R** **A**rchive **N**etwork ([GRAN](https://owi.usgs.gov/R/gran.html)),
requires a `repos` argument be specified.
For example, to install the **smwrData** package from GRAN, use

```{r, eval=FALSE}
install.packages("smwrData", repos = c(GRAN="https://owi.usgs.gov/R", CRAN="https://cran.r-project.org/"))
```

Notice that a CRAN repository was also included, this ensures that
package dependencies residing on CRAN will also be installed.
...
The `file.choose` function will allow you choose the ".zip" file interactively.
...
Let's install the **AnomalyDetection** package from its GitHub [repository](https://github.com/twitter/AnomalyDetection).
...
For example, to load the **viridis** package use the command
...
To get more information on any specific package, for example **sp**, the command is
...
And to open the help documentation for a specific function, for example `bbox`, the command is
...
It appears R is unable to find the help documentation for `bbox` because it belongs to the **sp** package,
and the **sp** package is not loaded in our current R session.
At this point, you can either do a fuzzy match of the function name using the `??bbox` command
(as recommended in the error message and appropriate when you don't know the package name),
load the **sp** package using the `library` function and rerun `?bbox`,
...
* [R for cats](http://rforcats.net/) is a lighthearted introduction to
  the R-programming language.

* [R-bloggers](https://www.r-bloggers.com/) is a blog aggregator of content contributed by
  bloggers who write about R.

* [StackOverflow](http://stackoverflow.com/questions/tagged/r) is a discussion forum for
...
4. Open a task view of your choosing and explore available packages.

### Closing R

Every time you terminate an R session,
by either typing `q()` at the command prompt or manually shutting down the R Console by point and click,
...
Recall that you can use the `ls` function to list these R objects.
...
If you say *Yes*, the `save.image` and `savehistory` functions are run,
resulting in files called ".RData" and ".Rhistory" being written to your working directory.
...
* SAS and R blog -- [another look at CRAN Task Views](http://sas-and-r.blogspot.com/2011/05/another-look-at-cran-task-views.html)

* Ohio Supercomputer Center website -- [HOWTO: Install Local R Packages](https://www.osc.edu/documentation/howto/install-local-R-packages)
